# ABOUTME: FSDP2 + QLoRA distributed training script for Challenge B.
# ABOUTME: Launched via accelerate/torchrun on 2x T4 GPUs.

"""
FSDP2 + QLoRA Training Script for Llama 3.1 8B

This script implements distributed training using:
- FSDP2 (Fully Sharded Data Parallel v2) via accelerate
- QLoRA (4-bit quantization + LoRA adapters) via bitsandbytes + peft
...
def parse_args():
    ...
    parser.add_argument(
        "--model_name",
        type=str,
        # Use base model for FSDP2 - pre-quantized models don't have correct quant_storage
    ...
    parser.add_argument(
        "--single_gpu_baseline",
        action="store_true",
        help="Run single GPU baseline for loss comparison",
    )
    parser.add_argument(
        "--use_torch_compile",
        action="store_true",
...
def setup_model_for_training(model, use_gradient_checkpointing=True, is_fsdp=False):
    """Configure model for training with proper gradient settings."""
    from peft import get_peft_model

    lora_config = get_lora_config()
    model = get_peft_model(model, lora_config)

    # Set requires_grad correctly
    with torch.no_grad():
    ...
    # Don't enable gradient_checkpointing when using FSDP - it handles it via activation_checkpointing
    if use_gradient_checkpointing and not is_fsdp:
...
def run_single_gpu_baseline(args):
    """Run single GPU training to establish baseline loss curve."""
    print("=" * 60)
    print("Running SINGLE GPU BASELINE for loss comparison")
    print("=" * 60)
    ...
    model = setup_model_for_training(model, args.use_gradient_checkpointing)
    ...
    training_args = SFTConfig(
        ...
        logging_steps=1,
        output_dir=f"{args.output_dir}_single_gpu",
        seed=3407,
    ...
    print("\n" + "=" * 60)
    print("SINGLE GPU BASELINE RESULTS")
    print("=" * 60)
...
def run_fsdp2_training(args):
    ...
    # Initialize accelerator (FSDP2 config comes from accelerate config/env)
    ...
    # Build training args - optionally enable torch.compile via TrainingArguments
