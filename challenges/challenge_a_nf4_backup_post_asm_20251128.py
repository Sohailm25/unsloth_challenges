...
def assert_same(x, y, line, dtype):
    expected_dtype = torch.float16 if (major_version < 8 and dtype == torch.bfloat16) else dtype
...
def assert_correct_bnb(weight, dtype):
    ...
    if major_version < 8 and dtype == torch.bfloat16:
        assert(weight.weight.quant_state.dtype == torch.float16)
    else:
...
class MLP(nn.Module):
    def __init__(self, hd = 4096, m = 14336, dtype = torch.float16):
        ...
        self.gate_proj.weight.quant_state.dtype = target_dtype
        self.up_proj  .weight.quant_state.dtype = target_dtype
        self.down_proj.weight.quant_state.dtype = target_dtype
...
def mlp_dequantize(X, mlp, fx):
    buffers = getattr(fx, "_buffers", None)
    if buffers is None:
        buffers = {}
    ...
    def get_buf(layer):
        ...
        key = id(layer)
        buf = buffers.get(key)
        needed = math.prod(qs.shape)
        if (
            buf is None
            or buf.numel() != needed
            or buf.device != layer.weight.device
            or buf.dtype != qs.dtype
        ):
            cached = getattr(qs, "_cached_out", None)
            if (
                cached is None
                or cached.numel() != needed
                or cached.device != layer.weight.device
                or cached.dtype != qs.dtype
            ):
                cached = torch.empty(needed, device=layer.weight.device, dtype=qs.dtype)
                qs._cached_out = cached
            buf = cached
            buffers[key] = buf
        return buf
    ...
    def call_fx(layer):
        buf = get_buf(layer)
        try:
            return fx(layer, out=buf)
        except TypeError:
            return fx(layer)

    a = call_fx(mlp.up_proj)
    b = call_fx(mlp.gate_proj)
    c = call_fx(mlp.down_proj)
    fx._buffers = buffers
    return a, b, c
...
def test_dequantize(dequantize_fx):
    ...
    for (bsz, qlen, hd, m, seed, dt) in options:
        ...
        dt_exec = torch.float16 if (major_version < 8 and dt == torch.bfloat16) else dt
        mlp = MLP(hd = hd, m = m, dtype = dt_exec)
        X = torch.randn((bsz, qlen, hd), device = "cuda", dtype = dt_exec)
...
def _your_dequantize_nf4_kernel(
    ...
    # FMA: code_val * scale + offset (Triton doesn't support runtime getattr)
    ...
    # Strided even/odd store (compile-safe on 2.3.1)
...
def _your_dequantize_nf4(
    ...
    *,
    ...
    if use_cache_eviction:
        evict = getattr(quant_state, "_cached_evict", None)
        if evict is None or evict.numel() != weight_flat_u8.numel() or evict.device != device:
            evict = torch.empty_like(weight_flat_u8)
            quant_state._cached_evict = evict
    else:
        evict = _DUMMY_EVICT.get(device)
        if evict is None or evict.device != device:
            evict = torch.empty(1, device=device, dtype=torch.uint8)
            _DUMMY_EVICT[device] = evict
    ...
    if out_tensor is not None:
        if out_tensor.numel() < n_weights or out_tensor.device != device:
            raise RuntimeError("Provided out_tensor has wrong size or device.")
        if out_tensor.dtype != out_storage_dtype:
            raise RuntimeError("Provided out_tensor dtype mismatch.")
        out_flat = out_tensor.view(-1)
    else:
        cached_out = getattr(quant_state, "_cached_out", None)
        if (
            cached_out is None
            or cached_out.numel() != n_weights
            or cached_out.device != device
            or cached_out.dtype != out_storage_dtype
        ):
            cached_out = torch.empty(n_weights, device=device, dtype=out_storage_dtype)
            quant_state._cached_out = cached_out
        out_flat = cached_out.view(-1)
    ...
    if emulate_bf16:
        # on sm75 we store fp16 as the final dtype
        if out_tensor is not None and out_tensor.dtype == torch.float16 and out_tensor.device == device:
            out_tensor.copy_(out_flat)
            out_flat = out_tensor
        else:
            out_flat = out_flat.to(torch.float16)
...
def your_dequantize_nf4(
    ...
    *,
    ...
    maj, minr = torch.cuda.get_device_capability(weight.weight.device)
    ...
    n_packed = weight.weight.data.numel()
